...
# Create computational nodes for the network
x_node = Input()
y_node = Input()

# input layer
w1 = Parameter(np.random.randn(input_mlp, hide) * 0.5)  # Weights
b1 = Parameter(np.zeros(hide))  # Biases
h1 = Linear(x_node, w1, b1)
a1 = Sigmoid(h1)  # Activation

#  hidden layer
w2 = Parameter(np.random.randn(hide, hide) * 0.5)
b2 = Parameter(np.zeros(hide))
h2 = Linear(a1, w2, b2)
a2 = Sigmoid(h2)

# Output layer
w3 = Parameter(np.random.randn(hide, out) * 0.5)
b3 = Parameter(np.zeros(out))
h3 = Linear(a2, w3, b3)
output = Sigmoid(h3)  # Sigmoid for binary classification

# Define the loss function (Binary Cross-Entropy)
loss = BCE(y_node, output)

# Define the computation graph
graph = [x_node, w1, b1, h1, a1, w2, b2, h2, a2, w3, b3, h3, output, loss]
trainables = [w1, b1, w2, b2, w3, b3]

# Helper functions
def forward_pass(graph):
    """Performs a forward pass through the computation graph."""
    for node in graph:
        node.forward()
...
def sgd_update(trainables, learning_rate):
    """Updates trainable parameters using stochastic gradient descent."""
    for t in trainables:
...
for epoch in range(epochs):
    ...
    for i in range(0, X.shape[0], batch_size):
        ...
        # Set input and output values for the batch
