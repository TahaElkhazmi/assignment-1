...
class Linear(Node):
    ...
    def forward(self):
        self.value = np.dot(self.x.value , self.w1.value) + self.w0.value

    def backward(self):
        self.gradients[self.x] = np.dot(self.outputs[0].gradients[self],self.w1.value.T)
        self.gradients[self.w1] = np.dot(self.x.value.T,self.outputs[0].gradients[self])
        self.gradients[self.w0] = np.sum(self.outputs[0].gradients[self],axis=0)
